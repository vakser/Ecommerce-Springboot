    ...
    public String addNewAdmin(@Valid @ModelAttribute("adminDto") AdminDto adminDto, BindingResult result, Model model) {
        try {
            ...
            if (admin != null) {
                ...
                model.addAttribute("emailError", "The email has been already registered!");
                return "register";
            }
            if (adminDto.getPassword().equals(adminDto.getRepeatPassword())) {
                ...
                model.addAttribute("adminDto", adminDto);
                model.addAttribute("passwordError", "Your passwords are not the same! Check again!");
                System.out.println("password not the same!");
                return "register";
            }
        } catch (Exception e) {
            e.printStackTrace();
            model.addAttribute("serverError", "Some error on the server!");
        }
